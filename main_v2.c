...
int main(int argc, char *argv[]) {
  ...
  server *local_serv = NULL; //pointer to the local server. -- previously knowmn just as serv
  server *new_serv = NULL; //to handle any node that enters the ring

  server *serv = NULL; // just to use on functions that are not complete
  ...
  while(strcmp(cmd, "exit\n") != 0){
    ...
    if (FD_ISSET(0, &readfds))
    {
      ...
      if(strcmp(token, "new") == 0)
      {
        ...
        else if(local_serv != NULL) printf("Cannot create a new ring!\n");
        else
        {
          /* ring creation stuff here */
          local_serv = newr(atoi(args[1]), ip, port); /* new ring/server creation */
          printf("A new ring has been created!\n");
          init_tcp_server(port, &local_serv, fd_parent); /* The TCP server initialized */
        }
        free(args);
      }
      else if(strncmp(token, "entry", 5) == 0)
      {
        ...
        else if(local_serv == NULL) printf("No ring created!\n");
        else
        {
      ...
      else if(strncmp(token, "sentry", 5) == 0)
      {
        ...
        else
        {
          ...
          //update the information that the entree has on its future successor -- operations made on the node that enters
          if(!update_state(&new_serv, atoi(args[1]), atoi(args[2]), args[3], args[4])) printf("Key <i> is not the local!\n");
          else
          {
            /* TCP session with succ, I'm client */
            fd_tcpC = init_tcp_client(&new_serv, &readfds);

            // in init_tcp_client the CLIENT is the node that enters the ring and the SERVER is the local node - the parent.
            // a msg is sent from the CLIENT to the SERVER with NEW ...
            // fd_pred of the parent node is the fd of the CLIENT that was just initialized.

            set_fd_pred(&local_serv, fd_tcpC);
        ...
        if(local_serv == NULL) printf("No ring created!\n");
        else {
